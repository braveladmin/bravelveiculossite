...
function Band({ src, alt, top, height, borderBottom, children }: {
  src: string
  alt: string
  top: string | number
  height: string
  ...
  return (
    <div
      ...
      style={{ top, left: 0, right: 0, height, ...(borderBottom ? { borderBottom: "2px solid #cc1111" } : {}) }}
    >
      <img
        src={src}
        alt={alt}
...
export function StoryCollagePreview({ vehicle }: Props) {
  ...
  return (
    <div className="media-preview story">
      ...
      <Band src={fotoExterna1} alt={vehicle.name} top={0} height="33.333%" borderBottom>
        <div
      ...
      {/* Badge Novidade — centralizado sobre a linha vermelha entre Banda 1 e 2 */}
      {vehicle.isNew && (
        <div
          className="absolute inset-x-0 z-20 flex justify-center"
          style={{ top: "calc(33.333% - 1px)", transform: "translateY(-50%)" }}
        >
          <span
      ...
      <Band src={fotoInterior} alt={`${vehicle.name} - interior`} top="33.333%" height="33.334%" borderBottom>
        <div
          className="absolute inset-x-0 bottom-0"
      ...
      <Band src={fotoExterna2} alt={`${vehicle.name} - traseira`} top="66.667%" height="33.333%">
